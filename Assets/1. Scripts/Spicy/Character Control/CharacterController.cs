...
public class CharacterController : MonoBehaviour, ICharacterController
{
    ...
    public void Attack()
    {
        if(visualPlayer.globalData.player.attackDuration <= 0f)
        {
            visualPlayer.globalData.audioList.PlaySoundEffect(transform, "swing");

            visualPlayer.SetSecondaryAnimation(AnimationType.ATTACK);

            visualPlayer.weaponHitboxObject.SetActive(true);

            float duration = visualPlayer.GetDurationOfAnimation(AnimationType.ATTACK);
            if(duration > 0f)
            {
                visualPlayer.globalData.player.attackDuration = duration + (Time.deltaTime * 4f);
            }
        }
    }
        ...
        switch(currentCharacterState)
        {
            case CharacterState.Default:
            {
                ...
                if(motor.GroundingStatus.IsStableOnGround)
                {
                    ...
                    {
                        visualPlayer.SetPrimaryAnimation(AnimationType.IDLE);
                        visualPlayer.animationSpeed = 1f;
                    }
                    else
                    {
                        visualPlayer.SetPrimaryAnimation(AnimationType.WALKING);
                        visualPlayer.animationSpeed = 1.5f;
                    }
                }
                // Air movement
                else
                ...
                {
                    visualPlayer.SetPrimaryAnimation(AnimationType.FALLING);
                }
